...
vec4 CalculatePointLight(vec3 aWorldSpaceNormal, vec3 aWorldSpacePixelPosition,vec4 aMaterialColor,vec4 aLightColor,vec4 aSpecularColor, float aMaterialReflectivity,vec3 aWorldSpaceLightPosition,vec3 aConstantLinearQuadraticAttenuation) 
{
   ...
   //Calculate the half vector, or the direction toward the fake specular light
	...
	return (finalDiffusedColor+finalSpecularColor);//Return the diffused and specular color that this light will have
}
...
vec4 CalculateDirectionalLight(vec3 aWorldSpaceNormal, vec3 aWorldSpacePixelPosition,vec4 aMaterialColor,vec4 aLightColor,vec4 aSpecularColor, float aMaterialReflectivity,vec3 aLightFacingDirection) 
{
   ...
   //Calculate the half vector, or the direction toward the fake specular light
	...
	//Ambient light
	vec4 finalAmbient=aLightColor*u_ambientPercentage;

	return (finalDiffusedColor+finalSpecularColor+finalAmbient);//Return the diffused and specular color that this light will have
}
	...
	vec3 aLightFacingDirection, float aAngleRange) 
{
   ...
   //Calculate the half vector, or the direction toward the fake specular light
	...
	directionFromLight=directionTowardLight*-1.0;
	...
	angleBetweenLightPixel=angleBetweenLightPixel*180.0/3.14159;
	...
	{	
		...
		diffusePercentage=0.0;
		specularPercentage=0.0;
	}
...
void main()
{
	...
	for(int i=0;i<u_NumberOfLights;i++)
	{
		...
		{
			tempColor=CalculatePointLight(worldSpaceNormal,worldSpacePixelPosition,materialColor,lightColor,specularColor,materialReflectivity,worldSpaceLightPosition,constantLinearQuadraticAttenuation);
		}
		else if (lightType==light_directional)//Directional light
		{
			tempColor=CalculateDirectionalLight(worldSpaceNormal,worldSpacePixelPosition,materialColor,lightColor,specularColor,materialReflectivity,direction);
		}
		else//Spot light
		{
			tempColor=CalculateSpotLight(worldSpaceNormal,worldSpacePixelPosition,materialColor,lightColor,specularColor,materialReflectivity,worldSpaceLightPosition,constantLinearQuadraticAttenuation,direction,angleRange);
	...
	//Apply ambient percentage
	finalColor*=ambientPercent;

	gl_FragColor = finalColor;
    //Get the final color the object will have.
	gl_FragColor.a=materialColor.a;
    //Reset the alpha so that it never changes
