...
GLvoid ResizeGLScene(GLsizei width, GLsizei height)
{
    ...
    if( g_pGame )
        g_pGame->OnSurfaceChanged( width, height );
}
...
void GenerateInputEvents(double aDelta)
{
    for( int i=0; i<256; i++ )
    {
        if( g_KeyStates[i] == true && g_OldKeyStates[i] == false )
            g_pGame->HandleInput(InputEvent(InputEventType_Key, InputEventState_Down, i, 0, 0), aDelta);
        else if( g_KeyStates[i] == true )
            g_pGame->HandleInput(InputEvent(InputEventType_Key, InputEventState_Held, i, 0, 0), aDelta);
        else if( g_KeyStates[i] == false && g_OldKeyStates[i] == true )
            g_pGame->HandleInput(InputEvent(InputEventType_Key, InputEventState_Up, i, 0, 0), aDelta);
    }

    int mx, my;
    GetMouseCoordinates( &mx, &my );

    g_pGame->HandleInput( InputEvent( InputEventType_Mouse, InputEventState_Max, -1, mx, my ),aDelta );

    for( int i=0; i<3; i++ )
    {
        if( g_MouseButtonStates[i] == true && g_OldMouseButtonStates[i] == false )
            g_pGame->HandleInput(InputEvent(InputEventType_Mouse, InputEventState_Down, i, mx, my), aDelta);
        else if( g_MouseButtonStates[i] == true )
            g_pGame->HandleInput(InputEvent(InputEventType_Mouse, InputEventState_Held, i, mx, my), aDelta);
        else if( g_MouseButtonStates[i] == false && g_OldMouseButtonStates[i] == true )
            g_pGame->HandleInput(InputEvent(InputEventType_Mouse, InputEventState_Up, i, mx, my), aDelta);
    }
}
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
    ...
    {
        g_KeyStates[i] = false;
        g_OldKeyStates[i] = false;
    }

    for( int i=0; i<3; i++ )
    {
        g_MouseButtonStates[i] = false;
    }
	
    // Create Our OpenGL Window
    if( !CreateGLWindow( "GAM1542/GAM1532_S3_ChavezMixco_Alvaro", SCREEN_WIDTH, SCREEN_HEIGHT, 32, 31, 1, false ) )
    {
        return 0;
    }
    ...
    pGame->LoadContent();
    pGame->OnSurfaceChanged( SCREEN_WIDTH, SCREEN_HEIGHT );
    ...
    while( !done )
    {
        ...
        for( int i=0; i<3; i++ )
        {
            g_OldMouseButtonStates[i] = g_MouseButtonStates[i];
        }
        ...
        if( g_WindowIsActive )
        {
            ...
            {
                GenerateInputEvents(timepassed);
                pGame->Update( timepassed );
                pGame->Draw();
